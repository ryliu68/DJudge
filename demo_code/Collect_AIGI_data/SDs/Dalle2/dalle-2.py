...
SAVE_PATH = "/home/xxx/work/datasets/COCO_2017/AIGC_EVAL/full/Dalle2"
IMAGE_SIZE = 512
...
def t2i(prompt):
    ...
    response = openai.images.generate(
        prompt=prompt, model=model, size="512x512")
...
def i2i(image_path):
    ...
    response = openai.images.create_variation(
        image=image, model=model, n=1, size='512x512')
...
def ti2i(prompt, image_path):
    ...
    response = openai.images.edit(
        image=image, prompt=prompt, mask=mask, model=model, n=1, size='512x512')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-T', '--type', type=str, required=True, choices=["T2I", "I2I"],
                        help='')  #
